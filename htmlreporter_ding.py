...
class HtmlReport:
    ...
    def generate_html(self, file):
        ...
        query = ('SELECT project, testplan FROM ' + testcase_report_tb +\
                             ' WHERE executed_history_id like \'%' + executed_history_id + '%\''  ' GROUP BY project, testplan ORDER BY id ASC')
        logger.info("query%s",query)
        data = ""
        ...
        for row in result:
            ...
            query = ('SELECT id, executed_history_id, testcase_id,tc_external_id, testcase_name,'
                                 ...
                                 ' WHERE project=%s AND testplan=%s AND executed_history_id like \'%' + executed_history_id + '%\'' 'GROUP BY testcase_id, runtime ORDER BY id ASC')
            data = (project,testplan)
            result = testdb.select_many_record(query, data)
        ...
        for row in result:
            ...
            query = ('SELECT tc_external_id, testcase_name, project, testplan, case_exec_history_id, testcase_id FROM ' + testcase_report_tb + \
                     ' WHERE project=%s AND testplan=%s AND executed_history_id like \'%' + executed_history_id + '%\'' \
            ...
            for row in result:
                ...
                query= 'SELECT step_num, protocol_method, protocol, HOST, PORT, step_action, expected_results, runresult, reason, runtime ' \
                      ...
                      'AND executed_history_id like  \'%' + executed_history_id + '%\'' \
                      'GROUP BY step_num ' \
                      'ORDER BY step_num ASC'
                data = (project, testplan, case_id)
                result = testdb.select_many_record(query, data)
                for row in result:
                    if row[7] != 'pass':
