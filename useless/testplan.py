...
class TestPlan:
    ...
    def run_testplan(self, http):
        ...
        #executed_history_id = time.strftime('%Y%m%d%H%M%S', time.localtime())  # 计划流水记录编号
        for testcase_id_str in testcases_id_for_testplan:
            ...
            testcase_obj = TestCase(testcase_id, testcase_name, testcase_steps, testcase_isactive, self.testproject, testsuite_id, tc_external_id, preconditions)
            ...
            data = (executed_history_id, testcase_id, str(testcase_name), testsuite_name, self.testplan_name, self.testproject, 'Block', '0000-00-00 00:00:00', tc_external_id)
            ...
            case_executed_history_id = time.strftime('%Y%m%d%H%M%S', time.localtime())  # 计划流水记录编号
            testcase_run_result = testcase_obj.run_testcase(http, self.testplan_name, case_executed_history_id)
            ...
            data =  (testcase_run_result[0], run_time, case_executed_history_id, executed_history_id, testcase_id, self.testproject,
                     self.testplan_name, '0000-00-00 00:00:00')
